...
>
	<div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-md mx-0 sm:mx-4 overflow-hidden max-h-[80vh] flex flex-col">
		<!-- Header -->
		<div class="p-6 pb-3 flex-shrink-0 bg-gradient-to-b from-orange-50 to-white">
			<div class="flex items-start justify-between mb-3">
				<h2 class="text-xl font-bold text-stone-800 flex-1 pr-4">{beacon.content.title}</h2>
				<button
					onclick={onClose}
			...
			{#if beacon.content.description}
				<p class="mt-3 text-sm text-stone-600">{beacon.content.description}</p>
			{/if}
		</div>
		...
		<div class="px-6 py-3 flex gap-2 flex-shrink-0">
			<button
				...
				disabled={responding}
				class="flex-1 py-2.5 rounded-xl text-sm font-medium transition cursor-pointer disabled:opacity-50 {myResponse?.status === 'joining' ? 'bg-emerald-500 text-white' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100'}"
			>
			...
			<button
				...
				disabled={responding}
				class="flex-1 py-2.5 rounded-xl text-sm font-medium transition cursor-pointer disabled:opacity-50 {myResponse?.status === 'interested' ? 'bg-amber-500 text-white' : 'bg-amber-50 text-amber-700 hover:bg-amber-100'}"
			>
			...
			<button
				...
				disabled={responding}
				class="flex-1 py-2.5 rounded-xl text-sm font-medium transition cursor-pointer disabled:opacity-50 {myResponse?.status === 'declined' ? 'bg-stone-500 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}"
			>
		...
		<div class="flex-1 overflow-y-auto px-6 pb-6">
			{#if joining.length > 0}
				<div class="mb-3">
					<p class="text-xs font-medium text-emerald-600 uppercase tracking-wider mb-1.5">Going</p>
					<div class="flex flex-wrap gap-1.5">
						{#each joining as person}
							<span class="px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs">
								{person.displayName}
							</span>
						{/each}
					</div>
			...
			{#if interested.length > 0}
				<div>
					<p class="text-xs font-medium text-amber-600 uppercase tracking-wider mb-1.5">Interested</p>
					<div class="flex flex-wrap gap-1.5">
						{#each interested as person}
							<span class="px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 text-xs">
								{person.displayName}
							</span>
						{/each}
					</div>
			...
			{#if !responses.data || responses.data.length === 0}
				<p class="text-sm text-stone-400 text-center py-4">No responses yet — be the first!</p>
			{/if}
		</div>
	</div>
</div>
