...
## Session 4 — Performance Audit: Redundancy Cleanup & Efficiency Pass

### Motivation
After a big UI/UX day (music cards, overlay styling, glassmorphic dropdowns), ran a thorough performance audit across three axes: PixiJS rendering, Svelte+Convex backend, and bundle/config. Codebase is solid — no dead code, no unused deps, good security headers. But found 10 real wins in reducing redundant work, preventing leaks, and parallelizing queries.

### Backend: Parallelize `areFriends()`
- `areFriends()` ran 2 sequential DB queries (forward + reverse friendship check) — called on every canvas load via `objects.getByCanvas`
- Wrapped both in `Promise.all()` — cuts friendship check latency roughly in half
- Other callers (e.g. `getFriends`) already used `Promise.all` — this was the last holdout
...
- Updated all 5 object files to use `FONT_FAMILY` — single source of truth
...
- Added `pendingRAF` tracking — cancelled in `destroy()`, cancelled-and-restarted on new schedule
...
- Removed the immediate redraw — `scheduleHeightCheck()` handles everything, including a final sync when height stabilizes
...
| `src/lib/canvas/textStyles.ts` | NEW — `FONT_FAMILY` + `CURSOR_LABEL_STYLE` shared constants |
...
## Session 5 — Pre-Deployment Security Audit

### Motivation
Third full security audit before deployment. Three parallel scans covered: (1) auth/CSP/env config, (2) all Convex mutations/queries for RBAC gaps, (3) client-side/WebRTC/dependencies. Codebase was already well-hardened from two prior passes — found 5 real fixes and 2 CSP hardening items.
...
- Added access check in try/catch — silent return on failure (stale signal cron cleans up)

### Fix 4: Rate limit friend requests (friendships.ts)
- No limit on `sendRequest` — user could spam every friend code they discover
...
- No limit on objects per canvas — malicious user could spam-create hundreds, degrading performance
...
- Added `form-action 'self'` — prevents form submissions to external domains
- Added `frame-ancestors 'none'` — CSP equivalent of `X-Frame-Options: DENY` (modern browsers prioritize CSP)
...
## Session 6 — Rebrand to Orbyt + Business Model + Charity Research

### Rebrand: Astrophage → Orbyt
...
- Not yet deployed — landing page in progress
...
- Helps friends who want to hang out but don't know what to do — the beacon becomes a planning assistant
- Added to README tech stack and feature list

### Business Model: Donation-Based (Wikipedia/Signal Model)
- **No ads, no data sales, no attention exploitation — ever**
...
- **Signal Foundation**: ~$38M/year operating costs ($14M servers, $19M staff). Funded by donations + Brian Acton's $105M 0% loan (due 2068). Runs at a net loss — cautionary tale.
...
- No encrypted message routing overhead — just canvas state sync

### Charity Research: Three Focus Areas

#### 1. Protecting People Affected by AI
- **Authors Guild** — advocacy for writers against unauthorized AI training
- **SAG-AFTRA** — 160K+ performers, led 2023 AI protections strike
- **Concept Art Association** — visual artists in entertainment fighting AI displacement
- **Human Artistry Campaign** — coalition of 40+ orgs advocating for artist consent/licensing
- **Fairly Trained** — certifies AI developers who license training data ethically
- **Algorithmic Justice League (AJL)** — exposed racial/gender bias in facial recognition
- **EFF** — digital rights, AI privacy, surveillance, free expression (founded 1990)
- **EPIC** — AI regulation, transparency, private rights of action
- **DAIR Institute** (Timnit Gebru) — AI research rooted in marginalized communities
- **AI Now Institute** — social implications of AI, concentration of power in tech

#### 2. AI for Good
- **Climate Change AI** — nonprofit catalyzing ML for climate solutions, 600+ papers, 60+ countries
- **Open Climate Fix** — halved UK solar forecasting errors, fully open source
- **Wild Me (Wildbook)** — AI-powered wildlife identification, 188K+ individual animals tracked
- **Khan Academy (Khanmigo)** — AI tutoring that augments human teachers, 165M users

#### 3. Digital Rights & Public Policy
- **Center for AI and Digital Policy (CAIDP)** — AI governance, UN statements, human rights
- **Partnership on AI** — 126 partners across 16 countries, equal nonprofit/industry board
- **Center for AI Safety** — reducing societal-scale AI risks
- **Center for Democracy and Technology (CDT)** — AI policy from a civil liberties lens (since 1994)
...
- **Cons**: OCF (main non-software fiscal host) dissolved end of 2024 — cautionary tale. Fee stacking. Donation page lives on their domain, not yours. Platform dependency.
...
| File | Change |
|------|--------|
| `README.md` | Full rebrand (Astrophage → Orbyt), orbyt.life link, Business Model section, Music features, updated Security/Rate limits, Claude Opus 4.6 in tech stack, updated Project Status |
...
- In progress: Landing page (orbyt.life), AI-assisted beacons, deployment
