...
## tween.js v25 Beacon Rendering Fix

### Problem
Beacons created via CreateBeaconModal were invisible on the canvas. Data existed in canvasObjects table and was returned by the Convex query, but nothing rendered.

### Root Cause
tween.js v25 changed behavior — `new TWEEN.Tween()` no longer auto-adds tweens to the default group. The beacon's pop-in animation (`scale: 0 → 1` over 400ms) started at scale 0 and the tween never ran because `TWEEN.update()` only processes tweens in its group. Beacon stayed permanently invisible at scale 0.

### Debugging Process
1. Added console.log to `syncObjects` — confirmed 2 objects received (textblock + beacon)
2. Wrapped BeaconObject creation in try/catch — no errors thrown
3. Logged `beacon.container.scale` immediately and at 100ms/500ms/1000ms — stuck at 0
4. Checked `TWEEN.getAll()` — returned 0 active tweens
5. Confirmed: tweens not in the default group

### Fix
Added `TWEEN.add(tween)` after `.start()` in all tween creation sites:
- `BeaconObject.ts` — pop-in animation + pulse glow animation
- `StickerReaction.ts` — pop-in animation (same silent bug)

### Files Modified
- `BeaconObject.ts` — `TWEEN.add()` for pop-in + pulse tweens
- `StickerReaction.ts` — `TWEEN.add()` for pop-in tween

---

## Sticker Reactions for Visitors

### Problem
Sticker reactions (long-press → emoji picker) only worked for canvas owners. Visitors couldn't long-press because `makeDraggable()` (which bundles drag + long-press) was skipped when `editable: false`.

### Design Decision
Sticker reactions are social feedback — visitors reacting to the owner's content. The backend already allowed it (`addSticker` requires only `"viewer"` access), but the frontend blocked the interaction.

### Solution
- New `makeLongPressable()` function in `DragDrop.ts` — sets up long-press detection (500ms hold, <5px movement) without drag. Cursor stays `default`, `stopPropagation` prevents panning during hold.
- `TextBlock.ts` + `BeaconObject.ts` — when `editable: false`, call `makeLongPressable()` instead of skipping interaction entirely.

### Files Modified
- `DragDrop.ts` — new `makeLongPressable()` export
- `TextBlock.ts` — `else if (options.onLongPress)` branch for visitors
- `BeaconObject.ts` — same pattern
...
- Beacon rendering: fixed (tween.js v25 group registration)
- Sticker reactions: now available for visitors (long-press without drag)
